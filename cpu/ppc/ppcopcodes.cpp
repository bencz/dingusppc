...
void dppc_interpreter::ppc_icbi(uint32_t opcode) {
    // there is nothing translated to drop without a code generator, which
    // keeps this as cheap as the placeholder it replaces
    ...
    ea &= ~(PPC_ICACHE_LINE_SIZE - 1);

    // icbi is not allowed to fault, so translate the way the debugger does.
    // An address that does not map cannot be holding translated code anyway
    uint32_t phys_addr;
    if (mmu_translate_dbg(ea, phys_addr)) {
        ppc_code_cache_invalidate(phys_addr, PPC_ICACHE_LINE_SIZE);
    }
}
